...
> 盲审/验前事「怎么出题」的统一方法规范，vedic-reader 的验前事 与 vedic-rectifier 的 Step 5 盘外验证共用。
> 只含「如何生成验前事推断」的方法主体（候选池/四通道/工具箱/构造SOP/输出模板）；
> 不含 reader 特有的用户反馈处理（命中率话术/可信度标注/反轴防御）——各 skill 收尾自理。
> ⚠️ 维护依赖：抽取自 vedic-reader SKILL.md 的 Step 5 方法主体；reader 改动验前事方法时需同步本文件。
